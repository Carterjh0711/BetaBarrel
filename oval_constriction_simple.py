#oval_constriction_simple
#Use /net/software/containers/pyrosetta.sif
#ls -1 | wc -1 *pdb
#qlogin --mem=32g -c 1 -p cpu
...
def changeAA_packer(pose):
	#Might need an interactive node. Allows Amino Acids to change
	scorefxn = pyrosetta.create_score_function('ref2015')
	task = rosetta.core.pack.task.TaskFactory.create_packer_task(pose)
	packer = rosetta.protocols.minimization_packing.PackRotamersMover(scorefxn, task)
	packer.apply(pose)
...
def main():
	...
	for iteration in range(5000):
		...
		if (iteration+1) % 100 == 0:
			pose.dump_pdb(f"./output/{args.output}/sameAA_movethrough_5000xMin_{iteration+1}.pdb")
